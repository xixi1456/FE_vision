...
: yolo_(config_path, debug)
{
  ...
  use_yolo_detect_ = (yolo_name == "nndeploy");
  if (use_yolo_detect_) {
    // nndeploy：由 yolo_(NNdeployment) 内部完成推理与后处理，pop() 中同步调用 detect()
    tools::logger()->info(
      "[MultiThreadDetector] nndeploy backend: 推理收敛到 YOLO::detect() (pop 线程)!");
    return;
  }
...
{
  if (use_yolo_detect_) {
    // nndeploy：只缓存图像，推理放 pop() 线程
    queue_.push({img.clone(), t, ov::InferRequest()});
    return;
  }
